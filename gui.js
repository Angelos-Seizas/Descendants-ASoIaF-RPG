...
/**
* RPGUI Initialisation
* This code initialises the RPGUI framework, serving as the first file included in the complied JavaScript.
*/
...
// If set to true, RPGUI will be initialized as soon as the page loads.
// If set to false, you will need to manually call RPGUI.init() to initialize RPGUI.
...
// Initialise the RPGUI framework and all associated components.
RPGUI.init = function()
{
...
// List of Functions for Initialisation
RPGUI.__init_list = [];

// Add a Function to Initialization List
// This function allows you to add a callback function to the initialization list.
// The functions in this list will be executed after RPGUI is fully loaded.
RPGUI.on_load = function(callback)
{
	// // If RPGUI was already initialized, call the callback function immediately
	if (RPGUI._was_init) {callback();}
	
	// Add the callback to the init list for execution later
	RPGUI.__init_list.push(callback);
}
/**
 * Unified and Simplified JavaScript Access
 * This section introduces a mechanism for streamlined JavaScript access to customized elements.
 * It aims to provide a user-friendly interface for working with RPGUI elements.
 */
...
var _checkbox_class = "desc-checkbox";

// create a desc-checkbox from a given element.
...
RPGUI.on_load(function()
{
	...
	var contents = document.getElementsByClassName("desc-content");
...
* This will replace automatically every <input> element that has the "desc-radio" class.
*/


// class name we will convert to special radio
var _radio_class = "desc-radio";

// create a desc-radio from a given element.
...
* This will replace automatically every <select> element that has the "desc-dropdown" class.
*/


// class name we will convert to dropdown
var _dropdown_class = "desc-dropdown";

// create a desc-dropdown from a given element.
...
function create_dropdown(elem)
{
	...
	if (elem.id) {select_header.id = elem.id + "-desc-dropdown-head"};
	RPGUI.add_class(select_header, "desc-dropdown-imp desc-dropdown-imp-header");
	RPGUI.insert_after(select_header, elem);

	// create the list to hold all the options
	var list = RPGUI.create_element("ul");
	if (elem.id) {list.id = elem.id + "-desc-dropdown"};
	RPGUI.add_class(list, "desc-dropdown-imp");
...
* This will replace automatically every <select> element that has the "desc-list" class.
*/


// class name we will convert to list
var _list_class = "desc-list";

// create a desc-list from a given element.
// note: element must be <select> with <option> tags that will turn into the items
...
function create_list(elem)
{
	...
	// create the list to hold all the options
	var list = RPGUI.create_element("ul");
	if (elem.id) {list.id = elem.id + "-desc-list"};
	RPGUI.add_class(list, "desc-list-imp");
	...
	(function(elem, all_items)
	{
		...
		function _on_change(elem)
		{
			for (var i = 0; i < all_items.length; ++i)
			{
				...
				{
					RPGUI.add_class(item, "desc-selected");
				}
				else
				{
					RPGUI.remove_class(item, "desc-selected");
				}
			}
		}
...
* This will replace automatically every <input> element that has the "desc-slider" class.
*/


// class name we will convert to special slider
var _slider_class = "desc-slider";

// create a desc-slider from a given element.
// note: element must be <input> of type "range" for this to work properly.
...
{
    RPGUI.add_class(element, "desc-cursor-" + cursor);
};
